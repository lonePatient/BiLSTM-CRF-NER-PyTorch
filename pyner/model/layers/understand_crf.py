...
'''
注意：该版本的crf
1. cpu的计算速度比gpu的计算速度快到3倍。
2. 所以其余层放在gpu训练，而crf层挡在cpu计算

该版本的CRF只适合理解，训练速度太慢了
'''
...
class CRF(nn.Module):
    ...
    def _reset(self):
        ...
        nn.init.uniform_(self.transitions, -0.1, 0.1) # nn.init.xavier_uniform_(self.transitions)
        self.transitions.data[self.end_tag, :] = -10000.0   # 表示从EOS->其他标签为不可能事件, 如果发生，则产生一个极大的损失
        self.transitions.data[:, self.start_tag] = -10000.0   # 表示从其他标签->SOS为不可能事件, 同上
        if self.use_cuda:
    ...
    def all_possible_path_score(self,features):
        ...
        # 初始化
        forward = Variable(torch.zeros(self.num_classes))     # 初始化START_TAG的发射分数为0
    ...
    def viterbi(self, features):
        time_steps = features.size(0)
        forward = Variable(torch.zeros(self.num_classes))  # START_TAG
